...
const ShopDetail = () => {
  ...
  useEffect(() => {
    ...
    axios
      .get(`https://localhost:7241/api/Products/detail_product?id=${idpro.id}`)
      .then((resp) => {
        setDetails(resp.data);

        axios
          .get(
            `https://localhost:7241/api/Products/Shop_Detail_Product?id=${resp.data.shopId}`
          )
          .then((responseShop) => {
        ...
        axios
          .get(
            `https://localhost:7241/api/Products/List_Image_ProductID?productId=${resp.data.productId}`
          )
          .then((res) => {
        ...
        axios
          .get(
            `https://localhost:7241/api/FeedBack?productID=${resp.data.productId}`
          )
          .then((resp) => {
        ...
        axios
          .get(
            `https://localhost:7241/api/Products/Product_ShopId?shopId=${resp.data.shopId}`
          )
          .then((resp) => {
  ...
  const addToCart = (productId) => {
    ...
    if (accessToken) {
      axios
        .post("https://localhost:7241/api/Order/Addtocart", addProductNow, {
          headers: {
  ...
  return (
    <div className="shopDetail" ref={scrollToTopRef}>
      {ShowRepAddFailed === true && (
        <div className="confirmation-modal">
          <div className="confirm-checkout-OK" style={{ width: "600px" }}>
            ...
            <button style={{ width: "200px" }} onClick={handleCheckoutOk}>
              OK
            </button>
          </div>
        </div>
      ...
      {showReportShop && (
        <UserReportShop
          shopId={infoShop.shopId}
          setShowReportShop={setShowReportShop}
        />
      )}
      <div className="detail-product">
        <div className="view-image">
          ...
          <div className="gallery-img">
            <div
              className={`gallery-pre ${galleryIndex <= 0 ? "disabled" : ""}`}
            ...
            <div className="gallery">
              {ListImg.slice(galleryIndex, galleryIndex + 4).map(
                (image, index) => (
                  <img
                    key={index}
                    src={image}
            ...
            <div
              className={`gallery-next ${
                galleryIndex >= ListImg.length - 4 ? "disabled" : ""
              }`}
        ...
        <div className="viewInfor">
          ...
          <div className="viewPrice">
            {details.discountPercent !== 0 && (
              <div className="price">
                <div className="don-vi">₫</div>
                {numeral(details.price).format("0,0")}
              </div>
            )}
            <div className="soldPrice">
              <div className="don-vi">₫</div>
              {numeral(details.soldPrice).format("0,0")}{" "}
            </div>
            {details.discountPercent !== 0 && (
          ...
          <div className="quantity">
            ...
            <div className="quantitySelect">
              <button className="buttonSelect" onClick={decreaseQuantity}>
                -
              </button>
              <input type="text" value={quantity} readOnly />
              <button
                className="buttonSelect"
                onClick={() => increaseQuantity(details)}
              >
                +
              </button>
            </div>
            <div className="quantityText">
              {details.quantity} sản phẩm có sẵn
            </div>
          </div>
      ...
      <div className="Shop-info">
        ...
        <div className="shop-1">
          <div className="shop-Name">{infoShop.shopName}</div>
        ...
        <div className="shopInfo-details">
          <div className="shopInfo-detail-info">
            ...
            <div className="detail-value">
              {moment(infoShop.createDate).format("DD-MM-YYYY")}{" "}
            </div>
          </div>
          <div className="shopInfo-detail-info">
      ...
      <div className="description-product">
        ...
        <div className="description-text">MÔ TẢ SẢN PHẨM</div>
        <div className="description-value">
          {details &&
            details.decription &&
            formatDescriptionValue(details.decription)}
        </div>
      </div>
      {/* Feeback Product */}
              ...
              <div className="feedback-info" key={index}>
                <div className="feedback-user">
                  ...
                  <div className="user-name-rate">
                    ...
                    <div className="user-date">{user.createDate}</div>
