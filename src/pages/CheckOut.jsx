...
const CheckOut = () => {
  ...
  const DeleteCartItem = () => {
    axios.post(
      "https://localhost:7241/api/Order/DeleteCarts",
      cartIdsToDelete,
      {
        headers: {
  ...
  const orderInfo = {
    items: Object.values(orderSelect).map((items) => ({
  ...
  const orderSelectID = Object.values(orderSelect).map(
    ...
    if (!addressData.length) {
      ...
      switch (paymentMethod) {
        case "Cash":
          setLoadDing(true);
          axios
            .post("https://localhost:7241/api/Order/Create", orderInfo, {
              headers: {
                Authorization: `Bearer ${accessToken}`,
              },
            })
            .then((res) => {
              ...
              axios
                .post(`https://localhost:7241/api/Order/Pay`, paymentSelect, {
                  headers: {
                    Authorization: `Bearer ${accessToken}`,
                  },
                })
      ...
      <div className="Cart-page-body">
              ...
              {!addressData.length ? (
                ...
                <div className="checkOut-info">
                  <div className="checkOut-name-phone">
                    <div className="checkOut-name">{firstAddress.nameRg}</div>
                    <div className="checkOut-phone">{firstAddress.phone}</div>
                  </div>
        ...
        <div className="checkOut-body-product">
          {Object.entries(shops).map(([shopId, { shopName, products }]) => (
            <div
              className="checkOut-log-shop"
              key={shopId}
              style={{ boxShadow: "0 2px 1px 0 rgba(0, 0, 0, .05)" }}
            >
              <div className="checkOut-nameShop">
                <div className="checkOut-nameShop-log">
                  <svg
                    ...
                    viewBox="0 0 24 24"
                    fill="currentColor"
                    className="iconShop-log-checkout"
                  >
              ...
              {products.map((product) => (
                <div className="checkOut-product" key={product.productId}>
                  ...
                  <div className="checkOut-ProductName">
                    {product.productName}
                  </div>
                  <div className="checkOut-Product-price">
                    <div className="checkOut-num">
                      <div className="don-vi">₫</div>
                      {numeral(product.soldPrice).format("0,0")}
                    </div>
                    <div
                      className="checkOut-num"
                      style={{ justifyContent: "center" }}
                    >
                      {product.quantity}
                    </div>
                    <div className="checkOut-subitem">
                      <div className="don-vi">₫</div>
                      {numeral(product.totalPrice).format("0,0")}
                    </div>
                  </div>
                </div>
              ))}
          ...
          <div className="pay-method">
            <div className="log-option-payment">
              <div className="method-option">
                ...
                />
                <img
                  src="https://lzd-img-global.slatic.net/g/tps/tfs/TB1ZP8kM1T2gK0jSZFvXXXnFXXa-96-96.png_2200x2200q75.png_.webp"
                  alt=""
                />
            ...
            <div className="pay-description-log">
              ...
              <div className="checkOut-Total">
                Tổng số lượng:
                <div className="num-payTotal-item">
                  {calculateTotalQuantity()}
                </div>
              </div>
              <div className="checkOut-Total">
                Tổng thanh toán:
                <div className="num-payTotal" style={{ display: "flex" }}>
                  <div className="don-vi">₫</div>
      ...
      {showNtPayMethod && (
        <div className="confirmation-modal">
          <div className="message-payMethod">
            <div className="ms-pay-text">
              Vui lòng chọn phuong thức thanh toán
            </div>
            <button className="ms-pay-button" onClick={handleShowNtPayMethod}>
              OK
            </button>
          </div>
        </div>
