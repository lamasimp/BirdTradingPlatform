...
const Cart = () => {
  ...
  const reloadData = () => {
    axios
      .get(`https://localhost:7241/api/Order/ViewCart`, {
        headers: {
  ...
  const toggleSelectProduct = (data) => {
    ...
    const finded = updatedOrderSelect.find(
      (u) => u.productId === data.productId
    );
    if (finded) {
      updatedOrderSelect = updatedOrderSelect.filter(
        (u) => u.productId !== data.productId
      );
    } else {
  ...
  const toggleSelectShop = (shopId) => {
    ...
    if (isAllExist) {
      let afterFilter = orderSelect.filter(
        (o) => !products.find((p) => p.productId === o.productId)
      );
  ...
  const UpdateItem = (data) => {
    ...
    if (data.quantityCart + data.quantity >= 1) {
      axios
        .post("https://localhost:7241/api/Order/UpdateQuantity", UpdateCart, {
          headers: {
            Authorization: `Bearer ${accessToken}`,
          },
        })
  ...
  const handleConfirmation = () => {
    let updatedOrderSelect = [...orderSelect];
    const finded = updatedOrderSelect.find(
      (u) => u.productId === productDeleteItem
    );
    if (finded) {
      updatedOrderSelect = updatedOrderSelect.filter(
        (u) => u.productId !== productDeleteItem
  ...
  return (
    <div className="Cart-page">
      <div className="Cart-page-body">
        <div className="Cart-scroll">
          <div className="Cart-page-inFor">
            ...
            <div className="cart-products-num">
              <div className="cart-inFor">Thao Tác</div>
            </div>
          </div>
          ...
          {ShowNull === true && (
            ...
            >
              <div>
                <img
                  style={{ height: "100px", margin: "0% 40%" }}
                  src="https://th.bing.com/th/id/R.243d0e0ebe06da1c163b355961f024a7?rik=%2f6oK8VKD8oY%2fmg&riu=http%3a%2f%2fwww.bulongviet.com%2fUploads%2fimages%2ficon_03.png&ehk=1%2fn9ChdNLIGH5HrtYoChSZvw5ST66JFRc7bI7B9OfhA%3d&risl=&pid=ImgRaw&r=0"
                  alt=""
                />
                <div
                  style={{
                      ...
                      {product.quantityProduct > 0 && (
                        <>
                          ...
                          <div className="cart-products-num">
                            ...
                            {numeral(product.priceProduct).format("0,0")}
                          </div>
                          ...
                          <div className="cart-products-num">
                            <div className="totalPrice-cart">
                              <div className="don-vi">₫</div>
                              {numeral(product.priceCart).format("0,0")}
                            </div>
                          </div>
                          <div className="cart-products-num">
                            <button
      ...
      <div className="cart-page-button"></div>

      <div className="product-reldate">
        <div className="product-text">CÓ THỂ BẠN CŨNG THÍCH</div>
      ...
      {showNotification && (
        <div className="confirmation-modal">
          <div className="confirm-checkout-OK">
            <div className="confirm-checkout-text">
              {" "}
