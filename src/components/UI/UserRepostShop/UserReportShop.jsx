...
const UserReportShop = ({ shopId, setShowReportShop }) => {
  ...
  useEffect(() => {
    axios
      .get(`https://localhost:7241/api/Customer/GetcatoryReport`)
      .then((responseShop) => {
  ...
  const handleReport = () => {
    if (accessToken) {
      ...
      if (categoriaI != 0 && DetailFB.length > 1) {
        axios
          .post(`https://localhost:7241/api/Customer/reportShop`, ReportData, {
            headers: {
  ...
  return (
    <div className="confirmation-modal">
      ...
      >
        <div id="title-feedback">Tùy Chọn Tố Cáo</div>
          ...
          {CateReport.map((cate) => (
            ...
            >
              <input
                class="red-input"
                type="checkbox"
                checked={categoriaI == cate.cateRpId}
                onChange={() => handleChange(cate.cateRpId)}
        ...
        <div id="title-feedback">Chi Tiết Tố Cáo</div>
        <div className="detail-feedback">
          <textarea
            maxlength="81"
            name="text"
            placeholder="dưới 80 lí tự"
          ...
          {DetailFB.length > 80 && (
            <label style={{ paddingLeft: "5px", color: "red" }}>
              {" "}
              Vui long nhập dưới 80 ký tự
            </label>
          )}
          {Dreport === true && (
            <label style={{ paddingLeft: "5px", color: "red" }}>
              {" "}
              Xin Vui Lòng Cung Cấp Đầy Đủ Để Gửi Tố Cáo
            </label>
          )}
        </div>
