...
const Received = () => {
  ...
  useEffect(() => {
    const ApiMain = () => {
      setOrderList([]);
      axios
        .get(`https://localhost:7241/api/Order/ToReceived/3`, {
          headers: {
  ...
  return (
    <div className="option-page-MyPurChase">
            ...
            .map((shop) => (
              <div className="toPay-list-log" key={shop.orderID}>
                  ...
                  <div className="toPay-nameShop">
                    <div className="toPay-nameShop-log">
                      ...
                      <div className="receButton">
                        <button
                          className="viewShop"
                          onClick={() => handleViewShopClick(shop.shopId)}
                        >
                          <svg
                            xmlns="http://www.w3.org/2000/svg"
                            fill="none"
                ...
                {shop.items.map((product) => (
                  <div className="toPay-product" key={product.productId}>
                    <div style={{ display: "flex" }}>
                      <img
                        src={product.firstImagePath}
                        alt=""
                        onClick={() => handleTabClick(shop)}
                      />
                      <div
                        className="toPay-ProductName"
                        onClick={() => handleTabClick(shop)}
                      >
                        <div className="toPay-name">{product.productName}</div>
                        <div className="toPay-quantity">
                          x{product.quantity}
                        </div>
                      </div>
                    </div>
                    ...
                    >
                      ...
                      >
                        <div className="don-vi-pay">₫</div>
                        {numeral(product.productPrice).format("0,0")}
                      </div>
                      <div className="toPay-numSoldPrice">
                        <div className="don-vi-pay">₫</div>
                    ...
                    {ShowFeedTable && (
                      <AddFeedback
                        ...
                        setShowFeedTable={setShowFeedTable}
                ...
                <div className="toPay-totalPay-log">
                  <div>Thành Tiền:</div>
                  <div id="toPay-totalPay">
                    <div className="don-vi-pay">₫</div>
                    {numeral(shop.totalPrice).format("0,0")}
                  </div>
                  <div>
                    <button
                      style={{
      ...
      {ShowBuyAgain && (
        <BuyAgain orderList={orderSelect} setShowBuyAgain={setShowBuyAgain} />
      )}
    </div>
  );
};
