...
const AddFeedback = ({
  ...
  const handleFeedback = () => {
    ...
    axios
      .post(`https://localhost:7241/api/FeedBack/Feedback`, formData, {
        headers: {
  ...
  return (
    <div className="confirmation-modal">
      ...
      >
        <div id="add-feedback-head">
          {" "}
          Đánh giá cho sản phẩm - {productName} -
        </div>
        ...
        <div className="star-rating">
          {[1, 2, 3, 4, 5].map((rating) => (
            ...
            >
              {" "}
              {rating <= starRating ? <BsStarFill /> : <BsStar />}
            </span>
          ))}
          {starRating == 1 && (
            <div id="text-of-star" style={{ color: "#5f5d5c" }}>
              {" "}
              Sản phẩm không tốt{" "}
            </div>
          )}
          {starRating == 2 && (
            <div id="text-of-star" style={{ color: "#d11f12" }}>
              {" "}
              Sản phẩm tạm ổn
            </div>
          )}
          {starRating == 3 && (
            <div id="text-of-star" style={{ color: "#ee6e06" }}>
              {" "}
              Sản phẩm ổn
            </div>
          )}
          {starRating == 4 && (
            <div id="text-of-star" style={{ color: "#0cdd86" }}>
              {" "}
              Sản phẩm tốt
            </div>
          )}
          {starRating == 5 && (
            <div id="text-of-star" style={{ color: "rgb(8, 158, 228)" }}>
              {" "}
              Sản phẩm rất tốt{" "}
            </div>
          )}
        </div>
        <div id="title-feedback">Đánh giá chi tiết</div>
        ...
        <div id="title-feedback">Ảnh đánh giá</div>
          ...
          <div className="addImage-feedback">
            {ImageFile.map((img, index) => (
              <div key={index} className="image-container-feedback">
                ...
                />
                <button
                  className="delete-button-imgFeedback"
        ...
        <div className="button-add-feedback">
          <button
            onClick={() => setShowFeedTable(false)}
            style={{
              backgroundColor: "#fff",
